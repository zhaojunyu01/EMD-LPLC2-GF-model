...
Fd_On=zeros(EMD_nx+1, EMD_ny+1);         % low-pass filtering
...
for i =1:(EMD_nx+1)
    for j =1:(EMD_ny+1)
        ...
        On(i, j) = OnRect(Fh(i, j), 0);
        Off(i, j) = OffRect(Fh(i, j), 0.05);
        
        Fd_On(i, j) =a_low(1, 1)*On(i, j)+a_low(1, 2)*Fd_On_before(i, j);                         % low-pass filtering
        Fd_Off(i, j) =a_low(1, 1)*Off(i, j)+a_low(1, 2)*Fd_Off_before(i, j);
    end
end
...
for i =1:EMD_nx
    for j =1:EMD_ny
        %------------ON pathway---------
        He_On(i, j)=Fd_On(i, j)*On(i, j+1);
        Hi_On(i, j)=On(i, j)*Fd_On(i, j+1);
        
        Ve_On(i, j)=Fd_On(i, j)*On(i+1, j);
        Vi_On(i, j)=On(i, j)*Fd_On(i+1, j);
