...
M = moviein(count);
for i = 1:count
    imagesc(LP(:,:,2,i)-LP(:,:,1,i));
    M(:,i) = getframe; 
end
title('horizontal visual motion in EMD');
%-----------------------------
subplot(1,2,2)
M = moviein(count);
for i = 1:count
    imagesc(LP(:,:,4,i)-LP(:,:,3,i));
    M(:,i) = getframe; 
end
title('vertical visual motion in EMD');
...
for i = 1:count
    imagesc(LPLC2(:,:,i));
    hold on;
    rectangle('Position',[50 25 100 100],'LineWidth',1,'EdgeColor','r');
    hold on;
    rectangle('Position',[75 50 50 50],'LineWidth',1,'EdgeColor','b');
    M(:,i) = getframe;
end
title('the LPLC2-Array response to stimulus');
end
